...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
start_wave = 1064; % center wavelength in nm
start_grating = 2;% grating 1 = 1800 grooves/mm Blz = 500 nm, grating 2 = 300 grooves/mm Blz = 750 nm

dynamic = 0; % keep capturing images if 1, or 1 image at a time if 0
exposure = 100; % in ms
gain = 1;
shutter = 3; % 1.) Always Closed 2.) Always Open 3.) Normal 4.) Open Before Trigger
shutterdelay = 0;
n_image = 1;
...
while i <= n_image
    ...
    if line_cam == 0
        intensity = sum(difference)/100;
    else
        intensity = difference(line_cam,:);   % finds the sums of each of the columns and then calculates the average value
    end
        % intensity is an array
    figure
    % plotting the intensity as a function of the calibrated wavelength  
    fig = plot(true_lambda,intensity);
